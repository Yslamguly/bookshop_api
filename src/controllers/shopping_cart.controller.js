...
exports.getUserShoppingCart = (req, res) => {
    ...
    db.select(`${tableName.books}.id`, `${tableName.books}.isbn`, `${tableName.books}.title`, `${tableName.books}.publication_year`, `${tableName.books}.selling_price`, `${tableName.books}.image`,
        ...
        .join(tableName.shopping_cart_item, function () {
            this.on(`${tableName.shopping_cart_item}.book_id`, '=', `${tableName.books}.id`)
        })
        .join(tableName.shopping_cart, function () {
            this.on(`${tableName.shopping_cart_item}.cart_id`, '=', `${tableName.shopping_cart}.id`)
        })
        .where(`${tableName.shopping_cart}.customer_id`, '=', customerId)
        .then(data => {
            data.length ? res.json(data) : res.status(200).json({message: 'You have no books in your cart'})
        })
...
exports.emptyShoppingCart = async(req, res) => {
    const {customerId} = req.params;

    const cart_id = await db.select('cart_id')
...
exports.deleteItemsFromShoppingCart = async(user_id) => {
    const cart_id = await db.select('cart_id')
        .from(tableName.shopping_cart)
